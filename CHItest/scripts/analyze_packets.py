...
Uses zip labels (P001…) as the analysis ID. Typed participant_id values
often collide and are not unique sessions.

Writes aggregate tables only — never copies raw prompts into markdown.

  CHITEST_PACKET_DIR=/path/to/zips-or-unpacked python3 CHItest/scripts/analyze_packets.py
"""
...
def packet_label(name: str) -> str:
    ...
    return f"P{m.group(1).zfill(3)}" if m else name.replace(" ", "")
...
def unpack_if_needed(root: Path, work: Path) -> Path:
    ...
    if dirs:
        return root
    zips = list(root.glob("*.zip")) + list(root.glob("*packet*.zip"))
    if not zips:
        raise SystemExit(f"No packets in {root}")
    work.mkdir(parents=True, exist_ok=True)
    for z in zips:
        dest = work / packet_label(z.name)
        dest.mkdir(exist_ok=True)
        if z.stat().st_size < 100:
            continue
...
def load_packet(folder: Path) -> dict:
    ...
    return {
        "folder": folder.name,
        "packet": packet_label(folder.name),
        "participant": p0,
        "tasks": tasks,
        "gens": gens,
        "autos": autos,
...
def issue_codes(issues) -> list[str]:
    ...
    for item in issues:
        if isinstance(item, dict):
            out.append(str(item.get("code") or item.get("flag") or item.get("id") or "issue"))
        else:
            out.append(str(item))
    return out
...
def build_rows(packets):
    ...
    for pkt in packets:
        ...
        issues = issue_codes(pkt["validation"].get("issues") or pkt["validation"].get("validation_issues") or [])
        tasks = pkt["tasks"]
        ...
        for g in gens:
            ...
            g["_concurrent"] = any(k in err for k in ("concurrent", "qps", "rate", "busy", "429", "限流", "繁忙"))
        ...
        for t in tasks:
            ...
            if not t.get("ended_at"):
                unfinished.append(t.get("stage") or t.get("task_id") or "?")
                continue
            self_by_block[block].append(num(t.get("self_alignment_rating")))
            result_by_block[block].append(num(t.get("result_alignment_rating")))
        ...
        recov = sum(1 for e in pkt["events"] if e.get("event_type") == "generation_recovery")
        all7 = len(ended) == 7
        ...
        if recov:
            notes.append(f"generation_recovery={recov}")
        if unfinished:
            notes.append("unfinished:" + "+".join(unfinished))
        rows.append(
            {
                "packet": pkt["packet"],
                ...
                "generation_recovery": recov,
                "sketch_sent_any": sketch_sent_any,
                "t2_loop_ok": t2_loop_ok,
                "p_norm_present": any((t.get("P_norm") or "").strip() for t in tasks),
                "self_raw": {k: [x for x in v if x is not None] for k, v in self_by_block.items()},
                "result_raw": {k: [x for x in v if x is not None] for k, v in result_by_block.items()},
...
def write_person_csv(rows: list[dict], path: Path) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        w.writeheader()
        for r in rows:
            w.writerow({k: r.get(k) if k != "notes" else ";".join(r.get("notes") or []) for k in fields})
...
def main():
    ...
    empty = [p.name for p in sorted(root.iterdir()) if p.is_dir() and not (p / "participants.csv").exists()]
    rows = build_rows(packets)
    formal = [r for r in rows if r["formal_efficacy"]]
    sens = [r for r in rows if r["complete7"]]
    out_csv = REPO / "docs" / "n20-person-table.csv"
    write_person_csv(rows, out_csv)
    print("packets_unpacked", len(rows), "empty_or_placeholder", empty)
    print("unique_start", len({r["started_at"] for r in rows}))
    ...
    print("sketch_sent", any(r["sketch_sent_any"] for r in rows))
    print("formal", [r["packet"] for r in formal], Counter(r["group_name"] for r in formal))
    print("complete7", [r["packet"] for r in sens], Counter(r["group_name"] for r in sens))
    print("wrote", out_csv)
    metrics = [
    ...
    for title, sample in [("formal", formal), ("complete7", sens)]:
        ...
        for metric, key in metrics:
            ...
            print(
                ...
                fmt(c["g_delta"]),
                "boot",
                f"{fmt(boot['lo'] if boot else None)}..{fmt(boot['hi'] if boot else None)}",
            )
