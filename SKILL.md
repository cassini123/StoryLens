---
name: you-dian-zi
description: "游点子 AI 助手，专注游戏设计、分镜创作、严肃游戏与VR教育应用的专业咨询。触发词包括：游戏设计、分镜、storyboard、严肃游戏、游戏化、game design、visual narrative、游戏化教学、游戏原型、level design、叙事设计、玩法机制、角色设计、UI/UX game、LoRA训练、风格迁移、AI辅助游戏创作、游戏美术、概念设计、cutscene、cinematic、游点子策划案、GDD。当用户询问游戏或分镜相关的设计建议、方法论评审或方案优化时触发此 Skill。"
agent_created: true
---

# Storyboard Generator — 游点子分镜生成器
...
详细交互规格见 `references/storyboard-language.md`。
...
**内置工具**：`assets/storyboard-generator.html` — 分镜生成器（三页面 Brutalism 设计系统应用）
...
**图片生成引擎**（已内置到分镜生成器）：
- **三重回退架构**：Buddy Cloud 云端生图 → SD WebUI API → Canvas Mock
...
- **5 种内置艺术风格**（sketch/render/anime/street/lora）：
  ...
  - ⬡ **自定义 LoRA** — 用户上传参考图训练风格
- 风格选择器底部上拉抽屉，带参考预览图（`object-fit:contain` 全图展示）
- 支持 LoRA 自定义风格训练（上传参考图 → 外部训练 → 引用 `<lora:name:weight>`）
...
`assets/storyboard-generator.html`（单文件应用，Brutalism 设计系统）
...
├── 分镜/镜头/视觉叙事问题?
│   ├── 需要生成分镜? → 引导使用 storyboard-generator.html
...
- `references/storyboard-language.md` — 分镜视觉语言完全手册（镜头语法/情绪曲线/转场）
...
- `assets/storyboard-generator.html` — 游点子分镜生成器主页面（完整单页应用，Brutalism 设计系统，~2900 行）
